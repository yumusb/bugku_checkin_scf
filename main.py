...
def checkin(s,bugkucookie):
    ...
    info = s.get("https://ctf.bugku.com/user/checkin",headers=headers,verify=False,timeout=40).text.split('<h3 class="m-t-30">')[1].split('</')[0]
    tip(info.strip())
def tip(text):
    data = {'desp':text}
...
def main(a,b):
    s = requests.Session()
    global headers
    githubloginurl = "https://github.com"+s.get("https://ctf.bugku.com/login",headers=headers,verify=False,timeout=40).text.split('https://github.com')[1].rsplit('"')[0]
    # 填写你的github cookie 理论只需以下两段就行，并且以下两段的值是相同的。
    githubcookie = "user_session=xxxxx; __Host-user_session_same_site=xxxxxxxx;"
    headers['cookie'] = githubcookie
    if(s.get("https://github.com/settings/profile",headers=headers,verify=False,timeout=40,allow_redirects=False).status_code != 200):
